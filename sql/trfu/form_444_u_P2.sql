CREATE PROCEDURE form_444_u_P2(IN startDate DATE)
  BEGIN
    SELECT
      ...
      trfu_blood_components.`id`            AS id,
    ...
    WHERE
      ...
      AND
      tests.`value` LIKE '%Положительный%'
    ...
    FROM
        ...
        INNER JOIN `trfu_blood_donation_requests_trfu_tests` trfu_blood_donation_requests_trfu_tests
          ON trfu_blood_donation_requests.`id` =
             trfu_blood_donation_requests_trfu_tests.`trfu_blood_donation_requests_id`
    ...
    WHERE
      ...
      AND date(wf_history.`startDate`) = startDate
      AND
      trfu_tests.`value` LIKE '%Положительный%'
