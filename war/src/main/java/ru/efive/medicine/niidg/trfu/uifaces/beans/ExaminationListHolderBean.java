...
@Named("examinationList")
@SessionScoped
public class ExaminationListHolderBean extends AbstractDocumentListHolderBean<ExaminationRequest> {
	...
	public List<ExaminationRequest> getAdmittedExaminationList() {
		List<ExaminationRequest> result = new ArrayList<>();
		try {
			result = sessionManagement.getDAO(ExaminationRequestDAOImpl.class, EXAMINATION_DAO).findDocumentsByStatus(5, filter, false,
				getPagination().getOffset(), getPagination().getPageSize(), getSorting().getColumnId(), getSorting().isAsc());
		}
		catch (Exception e) {
			e.printStackTrace();
