...
@Named("examinationPrimaryList")
@SessionScoped
public class ExaminationPrimaryListHolderBean extends AbstractDocumentListHolderBean<ExaminationRequest> {
	...
	protected int getTotalCount() {
		int result = 0;
		try {
			return new Long(sessionManagement.getDAO(ExaminationRequestDAOImpl.class, EXAMINATION_DAO).countDocumentByStatus(7, filter, false)).intValue();
		}
		catch (Exception e) {
			e.printStackTrace();
		}
	...
	protected List<ExaminationRequest> loadDocuments() {
		...
		try {
			result = sessionManagement.getDAO(ExaminationRequestDAOImpl.class, EXAMINATION_DAO).findDocumentsByStatus(7, filter, false,
