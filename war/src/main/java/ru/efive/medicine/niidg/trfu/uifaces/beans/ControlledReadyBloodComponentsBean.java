...
public class ControlledReadyBloodComponentsBean extends AbstractDocumentListHolderBean<BloodComponent> {
	...
	protected int getTotalCount() {
		int result = 0;
		try {
			result = new Long(sessionManagement.getDAO(BloodComponentDAOImpl.class, BLOOD_COMPONENT_DAO).countControlledComponentsByStatusId(filter, 3, false)).intValue();
		}
		catch (Exception e) {
			e.printStackTrace();
		}
	...
	protected List<BloodComponent> loadDocuments() {
		...
		try {
			result = sessionManagement.getDAO(BloodComponentDAOImpl.class, BLOOD_COMPONENT_DAO).findControlledComponentsByStatusId(filter, 3, false,
