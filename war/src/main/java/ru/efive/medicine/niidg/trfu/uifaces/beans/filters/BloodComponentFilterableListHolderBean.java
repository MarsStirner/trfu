...
public class BloodComponentFilterableListHolderBean extends AbstractFilterableListHolderBean<BloodComponent, BloodComponentsFilter> {
    ...
    private static final Logger logger = LoggerFactory.getLogger(BloodComponentFilterableListHolderBean.class);
    ...
    public List<FilterParameter> getNotNullFilterParameters() {
        ...
        if (StringUtils.isNotEmpty(fio)) {
            parameters.add(
                    new FilterParameter(
                            BloodComponentsFilter.FIO_TITLE, fio
                    )
            );
        }
        if (bloodGroupId != AbstractFilter.BLOOD_GROUP_NULL_VALUE) {
            BloodGroup bloodGroup = getDictionaryDAO().get(
                    BloodGroup.class, bloodGroupId
            );
            if (bloodGroup != null) {
                parameters.add(
                        new FilterParameter(
        ...
        if (rhesusFactorId != AbstractFilter.RHESUS_FACTOR_NULL_VALUE) {
            Classifier rhesusFactor = getDictionaryDAO().get(
                    Classifier.class, rhesusFactorId
            );
            if (rhesusFactor != null) {
                parameters.add(
                        new FilterParameter(
                                AbstractFilter.RHESUS_FACTOR_TITLE, rhesusFactor.getValue()
                        )
                );
            }
        }
        if (donationDateFrom != null) {
            parameters.add(
                    new FilterParameter(
                            AbstractFilter.DATE_FROM_TITLE, DateHelper.formatDateByPattern(
                            donationDateFrom, DateHelper.DATE_WITHOUT_TIME_PATTERN
                    )
                    )
            );
        }
        if (donationDateTo != null) {
            parameters.add(
                    new FilterParameter(
                            AbstractFilter.DATE_TO_TITLE, DateHelper.formatDateByPattern(
                            donationDateTo, DateHelper.DATE_WITHOUT_TIME_PATTERN
                    )
                    )
            );
        }
        if (expirationDateFrom != null) {
            parameters.add(
                    new FilterParameter(
                            AbstractFilter.DATE_FROM_TITLE, DateHelper.formatDateByPattern(
                            expirationDateFrom, DateHelper.DATE_WITHOUT_TIME_PATTERN
                    )
                    )
            );
        }
        if (expirationDateTo != null) {
            parameters.add(
                    new FilterParameter(
                            AbstractFilter.DATE_TO_TITLE, DateHelper.formatDateByPattern(
                            expirationDateTo, DateHelper.DATE_WITHOUT_TIME_PATTERN
                    )
                    )
            );
        }
        if (statusId != BloodComponentsFilter.BLOOD_COMPONENT_STATUS_NULL_VALUE) {
            ...
            parameters.add(
                    new FilterParameter(
                            AbstractFilter.STATUS_TITLE, statusName
                    )
            );
        }
        if (bloodComponentTypeId != BloodComponentsFilter.BLOOD_COMPONENT_TYPE_NULL_VALUE) {
            ...
            if (bloodComponentType != null) {
                parameters.add(
                        new FilterParameter(
                                AbstractFilter.BLOOD_COMPONENT_TYPE_TITLE, bloodComponentType.getValue()
                        )
                );
            }
        }
        if (makerId != BloodComponentsFilter.MAKER_NULL_VALUE) {
            Contragent contragent = getDictionaryDAO().get(
                    Contragent.class, makerId
            );
            if (contragent != null) {
                parameters.add(
                        new FilterParameter(
                                AbstractFilter.MAKER_TITLE, contragent.getFullName()
                        )
                );
            }
        }
        ...
        if (historyCreatedDateFrom != null) {
            parameters.add(
                    new FilterParameter(
                            AbstractFilter.DATE_FROM_TITLE, DateHelper.formatDateByPattern(
                            historyCreatedDateFrom, DateHelper.DATE_WITHOUT_TIME_PATTERN
                    )
                    )
            );
        }
        if (historyCreatedDateTo != null) {
            parameters.add(
                    new FilterParameter(
                            AbstractFilter.DATE_TO_TITLE, DateHelper.formatDateByPattern(
                            historyCreatedDateTo, DateHelper.DATE_WITHOUT_TIME_PATTERN
                    )
                    )
            );
        }
    ...
    @Override
    public List<BloodComponent> loadDocuments(
            int offset, int pageSize, BloodComponentsFilter filter
    ) {
    ...
    @Override
    public void formDocxContent(WordprocessingMLPackage pkg, File logoFile) throws Exception {
