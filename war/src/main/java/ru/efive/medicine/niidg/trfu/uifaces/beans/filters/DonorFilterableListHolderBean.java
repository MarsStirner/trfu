...
@SessionScoped
public class DonorFilterableListHolderBean extends
		AbstractFilterableListHolderBean<Donor, DonorsFilter> {
	...
	@Override
	public List<Donor> loadDocuments(int offset, int pageSize,
			DonorsFilter filter) {
		try {
			return sessionManagement.getDAO(DonorDAOImpl.class, "donorDao")
					.findDocuments(filter, offset, pageSize,
	...
	public List<FilterParameter> getNotNullFilterParameters() {
		...
		if (StringUtils.isNotEmpty(number)) {
			parameters.add(new FilterParameter(DonorsFilter.NUMBER_TITLE,
					number));
		}
		if (created != null) {
		...
		if (StringUtils.isNotEmpty(lastName)) {
			parameters.add(new FilterParameter(DonorsFilter.LAST_NAME_TITLE,
					lastName));
		}
		if (StringUtils.isNotEmpty(firstName)) {
			parameters.add(new FilterParameter(DonorsFilter.FIRST_NAME_TITLE,
					firstName));
		}
		if (StringUtils.isNotEmpty(middleName)) {
			parameters.add(new FilterParameter(DonorsFilter.MIDDLE_NAME_TITLE,
					middleName));
		}
		if (birth != null) {
		...
		if (StringUtils.isNotEmpty(documentNumber)) {
			parameters.add(new FilterParameter(
					DonorsFilter.DOCUMENT_NUMBER_TITLE, documentNumber));
		}
		if (StringUtils.isNotEmpty(documentSeries)) {
			parameters.add(new FilterParameter(
					DonorsFilter.DOCUMENT_SERIES_TITLE, documentSeries));
		}
		if (StringUtils.isNotEmpty(externalNumber)) {
			parameters.add(new FilterParameter(
					DonorsFilter.EXTERNAL_NUMBER_TITLE, externalNumber));
		}
		if (StringUtils.isNotEmpty(factAddress)) {
			parameters.add(new FilterParameter(DonorsFilter.FACT_ADDRESS_TITLE,
					factAddress));
		}
		if (categoryId != DonorsFilter.DONOR_CATEGORY_NULL_VALUE) {
			DonorCategory donorCategory = getDonorCategory(categoryId);
			if (donorCategory != null) {
				parameters.add(new FilterParameter(
						DonorsFilter.DONOR_CATEGORY_TITLE, donorCategory
								.getValue()));
			}
		}
		...
		if (pastQuarantineId != DonorsFilter.PAST_QUARANTINE_NULL_VALUE) {
			...
			if (pastQuarantine != null) {
				parameters.add(new FilterParameter(
						DonorsFilter.PAST_QUARANTINE_TITLE, pastQuarantine
								.getValue()));
			}
		}
		return parameters;
	...
	public DonorCategory getDonorCategory(int categoryId) {
		DonorCategory donorCategory = getDictionaryDAO().getDonorCategory(
				categoryId);
	...
	/**
	 * Получение списка объектов для признака "Прошедние карантинизацию" (да,
	 * нет).
	 * 
	...
	public void formXlsContent(Workbook wb, File logoFile) throws Exception {
		new DonorsXlsGenerator(wb, logoFile, this).formContent();
	}
	...
	public void formDocxContent(WordprocessingMLPackage pkg, File logoFile)
			throws Exception {
		new DonorsDocxGenerator(pkg, logoFile, this).formContent();
	}
	...
	@Override
	public DonorsFilter createFilterInstance() {
		return new DonorsFilter();
	}

	@Override
	public int getTotalCount(DonorsFilter filter) {
		try {
			long count = sessionManagement.getDAO(DonorDAOImpl.class,
					"donorDao").countDocument(filter);
