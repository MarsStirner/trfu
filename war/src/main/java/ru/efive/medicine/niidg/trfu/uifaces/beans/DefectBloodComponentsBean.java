...
@Named("defectBloodComponentList")
@SessionScoped
public class DefectBloodComponentsBean extends AbstractDocumentListHolderBean<BloodComponent> {
	...
	protected int getTotalCount() {
		int result = 0;
		try {
			result = new Long(sessionManagement.getDAO(BloodComponentDAOImpl.class, BLOOD_COMPONENT_DAO).countDocument(filter, -1, false)).intValue();
		}
		catch (Exception e) {
			e.printStackTrace();
		}
	...
	protected List<BloodComponent> loadDocuments() {
		...
		try {
			result = sessionManagement.getDAO(BloodComponentDAOImpl.class, BLOOD_COMPONENT_DAO).findDocuments(filter, -1, false,
