...
public class ProcessingBiomaterialListBean extends AbstractDocumentListHolderBean<Biomaterial> {
	...
	protected int getTotalCount() {
		int result = 0;
		try {
			result = new Long(sessionManagement.getDAO(MedicalOperationDAOImpl.class, MEDICAL_DAO).countBiomaterials(filter, 3, false)).intValue();
		}
		catch (Exception e) {
			e.printStackTrace();
		}
	...
	protected List<Biomaterial> loadDocuments() {
		...
		try {
			result = sessionManagement.getDAO(MedicalOperationDAOImpl.class, MEDICAL_DAO).findBiomaterials(filter, 3, false,
