...
public class PurchasedBloodComponentListHolderBean extends AbstractDocumentListHolderBean<BloodComponent> {
	...
	protected int getTotalCount() {
		int result = 0;
		try {
			result = new Long(sessionManagement.getDAO(BloodComponentDAOImpl.class, ApplicationHelper.BLOOD_COMPONENT_DAO).countPurchasedDocument(filter, false)).intValue();
		}
		catch (Exception e) {
			e.printStackTrace();
		}
