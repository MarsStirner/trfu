...
@ManagedBean(name = "controlledDefectBloodComponentList")
@ViewScoped
public class ControlledDefectBloodComponentsBean extends AbstractDocumentListHolderBean<BloodComponent> {
    ...
    protected int getTotalCount() {
        int result = 0;
        try {
            result = new Long(sessionManagement.getDAO(BloodComponentDAOImpl.class, BLOOD_COMPONENT_DAO).countControlledComponentsByStatusId(filter, -1, false)).intValue();
        } catch (Exception e) {
            e.printStackTrace();
        }
        return result;
    ...
    protected List<BloodComponent> loadDocuments() {
        ...
        try {
            result = sessionManagement.getDAO(BloodComponentDAOImpl.class, BLOOD_COMPONENT_DAO)
					.findControlledComponentsByStatusId(filter, -1, false, getPagination().getOffset(), getPagination
                            ().getPageSize(), getSorting().getColumnId(), getSorting().isAsc());
        } catch (Exception e) {
            e.printStackTrace();
        }
        return result;
