...
public class DonorListHolderBean extends AbstractDocumentListHolderBean<Donor> {
	...
	public List<Donor> getDocumentsByPhenotypes(List<Analysis> phenotypes, boolean searchBloodGroup, String bloodGroup, boolean searchRhesus, String rhesusFactor) {
		List<Donor> result = new ArrayList<Donor>();
		try {
			if (phenotypes != null && phenotypes.size() > 0) {
				result = sessionManagement.getDAO(DonorDAOImpl.class, ApplicationHelper.DONOR_DAO).findComponentsByPhenotypes(
						phenotypes, searchBloodGroup, bloodGroup, searchRhesus, rhesusFactor, "lastName,firstName,middleName", true);
			}
			else {
