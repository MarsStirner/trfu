    ...
    public static <T extends ProcessedData> Process getProcessByType(T t, ProcessUser user) {
        ...
        try {
            ...
            if (t.getType().equals("Examination")) {
                ...
                list = new ArrayList<Object>();
                list.add(t);
                invokeActivity.setInvokeInformation(
                        "ru.efive.medicine.niidg.trfu.wf.util.WorkflowHelper",
                        "initializeBloodComponentQualityControl",
                        list
                );
                activity.setParentAction(noStatusAction);
                        ...
                        try {
                            ...
                            if ((Long) scriptResult > 0) {
                                StringBuffer buffer = new StringBuffer(
                                        "Внимание!@В карантине находятся компоненты крови, полученные от этого донора.@"
                                );
                ...
                list = new ArrayList<Object>();
                list.add(t);
                activity.setInvokeInformation(
                        "ru.efive.medicine.niidg.trfu.wf.util.WorkflowHelper",
                        "checkAndUpdateQuarantinedBloodComponentsStatus",
                        list
                );
                activity.setParentAction(toStatusAction);
                activites.add(activity);
                        ...
                        try {
                            ...
                            for (Role currentRole : user.getRoles()) {
                                if (RoleType.ENTERPRISE_ADMINISTRATION.equals(currentRole.getRoleType()) ||
                                        RoleType.EXPEDITION.equals(currentRole.getRoleType())) {
                                    return true;
                                }
                            }
                ...
                List<IActivity> activites = new ArrayList<IActivity>();
                // Проверить что пока юзер редактировал заявку она не поменялась
                InvokeMethodActivity checkVolatile = new InvokeMethodActivity();
                List<Object> checkVolatileArgs = new ArrayList<Object>();
                checkVolatileArgs.add(t);
                checkVolatile.setInvokeInformation("ru.efive.medicine.niidg.trfu.wf.util.WorkflowHelper", "checkBloodComponentOrderNotChangedExternally", checkVolatileArgs);
                checkVolatile.setParentAction(toStatusAction);
                activites.add(checkVolatile);
                ...
                activitySetProp.setPropertyChanges(propertyChanges);
                activites.add(activitySetProp);
                //проставляем в качестве медсестры того пользователя, котрый выполнил преход
                if(PropertyUtils.getProperty(process.getProcessedData(), "staffNurse") == null) {
                    SetPropertyActivity setStaffNurseActivity = new SetPropertyActivity();
                    Map<String, Object> setStaffNurseActivityProps = new HashMap<String, Object>();
                    setStaffNurseActivityProps.put("staffNurse", process.getProcessUser().getUser());
                    setStaffNurseActivity.setPropertyChanges(setStaffNurseActivityProps);
                    activites.add(setStaffNurseActivity);
                }
                ...
                Collections.sort(
                        fromStatusActions, new Comparator<StatusChangeAction>() {
                            @Override
