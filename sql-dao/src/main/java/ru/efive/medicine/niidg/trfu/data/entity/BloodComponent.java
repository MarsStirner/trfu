...
public class BloodComponent extends IdentifiedEntity implements ProcessedData, Cloneable {
    ...
    @OneToMany(cascade = CascadeType.ALL)
    @JoinTable(name = "trfu_blood_component_tests",
            joinColumns = {@JoinColumn(name = "component_id")},
    ...
    @JoinTable(name = "trfu_blood_component_entry",
            joinColumns = {@JoinColumn(name = "component_id")},
            inverseJoinColumns = {@JoinColumn(name = "entry_id")})
    @LazyCollection(LazyCollectionOption.FALSE)
    private BloodDonationEntry factEntry;
    ...
    public String getFullNumber() {
        return purchased ? (getParentNumber() == null || getParentNumber()
                .equals("") ? getNumber() : getParentNumber() + "-" + getNumber()) : getParentNumber() + "-" + getNumber();
    }
    ...
    //TODO тесты для закупленных КК
    public List<Analysis> getTestList() {
        return purchased ? testList : null;
    }

    public void setTestList(final List<Analysis> testList) {
        if (purchased) {
           ...
           List<Analysis> result = new ArrayList<Analysis>(6);
            if(testList != null && !testList.isEmpty()) {
                for (Analysis current : testList) {
                    if (current.getType().getCategory().equals("Иммуносерология")) {
                        result.add(current);
                    }
                }
            }
